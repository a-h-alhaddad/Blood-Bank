  ...
  <aside class="sidenav navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-end me-3 rotate-caret  bg-gradient-dark" id="sidenav-main">
    <div class="sidenav-header">
      ...
      <a class="navbar-brand m-0" href=" https://demos.creative-tim.com/material-dashboard/pages/dashboard " target="_blank">
        ...
        <span class="me-1 font-weight-bold text-white">لوحة تحكم المتبرع</span>
      </a>
    </div>
    ...
    <div class="collapse navbar-collapse px-0 w-auto " id="sidenav-collapse-main">
      <ul class="navbar-nav">
        <li class="nav-item">
          <a class="nav-link active" href="{{ route('donor_list') }}">
        ...
        <li class="nav-item">
                <a class="nav-link" href="{{ route('edit_donor') }}">
                  <div class="text-white text-center ms-2 d-flex align-items-center justify-content-center">
                  </div>
    ...
    <div class="container mt-2">
      ...
      @if (count($donorRequests) > 0)
        <table class="table table-bordered">
          ...
          @foreach ($donorRequests as $d)
            <tr>
              <td>{{ $d->id }}</td>
              <td>{{ $d->blood_group->name ?? 'N/A' }}</td>
               <td>{{ $d->date}}</td> </tr>
          @endforeach
        </table>
    
        @if ($donorRequests->hasPages())
          <div class="d-flex justify-content-center">
            {{ $donorRequests->links() }} </div>
        @endif
    
      @else
        <p>No requests matching your blood group found.</p>
      @endif
  </main>
