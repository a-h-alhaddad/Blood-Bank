...
<body class="g-sidenav-show rtl bg-gray-200">
  <aside class="sidenav navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-end me-3 rotate-caret  bg-gradient-dark" id="sidenav-main">
    ...
    <div class="collapse navbar-collapse px-0 w-auto " id="sidenav-collapse-main">
      <ul class="navbar-nav">
        <li class="nav-item">
          <a class="nav-link" href="{{ route('blood_bank_request.create') }}">
        ...
        <li class="nav-item">
                <a class="nav-link active" href="{{ route('hospital_list') }}">
                  <div class="text-white text-center ms-2 d-flex align-items-center justify-content-center">
                  </div>
  ...
  <main class="main-content position-relative max-height-vh-100 h-100 border-radius-lg overflow-x-hidden">
    ...
    <div class="container mt-2">
        ...
        <div class="pull-left">
          ...
          <h3 class="mb-4" style="font-family: 'Cairo', sans-serif; ">سجل الطلبات</h3>
        ...
        @foreach ($Blood_bank_request as $request)

        <tr>

        <td>{{ $request->id }}</td>

        <td>{{ $request->blood_group->name ?? 'N/A' }}</td>
        ...
        <td>
          <span class="badge bg-{{ $request->status == 'accepted' ? 'success' : ($request->status == 'rejected' ? 'danger' : 'secondary') }}">
            {{ $request->status == 'accepted' ? 'مقبول' : ($request->status == 'rejected' ? 'مرفوض' : 'تتم المعالجة') }}
          </span>
        </td>
